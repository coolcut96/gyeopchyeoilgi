...
FORBIDDEN = [
 ...
 '섹트','out-of-sect','디그니티','프로펙션','피르다리아',
 '어센던트','도머사일','삼합','육합','원진','방합','삼형',
]
# 부분일치로 잘못 걸리는 흔한 일상어 — 검사 전에 지워서 오탐 방지
_SAFE = ['상관없','상관있','상관관계','상관도','상관성',
         '우수성','순수성','특수성','감수성','보수성','완수성','다양성','변화성','안정성',
         '신약성','일관성','일간지','역행하','역행해','역행할']
def leak_check(text):
    t = text
    for s in _SAFE:
        t = t.replace(s, '·')
    hits = [w for w in FORBIDDEN if w in t]
    if re.search(r'\d\s*하우스', t): hits.append('N하우스')
...
def build_prompt(brief, tier):
    ...
    return f"""당신은 사주와 별자리를 함께 읽는 성격 풀이 리포트의 문장을 다듬는 작가입니다.
...
3-1. 반복을 엄격히 피한다. ⑴ 가장 두드러진 강점(주 강점)은 리포트 전체에서 '딱 한 번만' 또렷하게 짚고, 뒤에서 같은 힘을 다른 말로 되풀이하지 않는다 — 한 번 짚었으면 다음은 다른 측면으로 넘어간다. ⑵ 한 리포트 안에서 같은 서술 표현('제법 안정적이에요', '앞장서서 밀어붙이는', '~쪽이에요' 같은 상투구)을 두 번 이상 재사용하지 말고, 영역마다 다른 동사·형용사로 묘사한다. ⑶ 각 영역(인정과 명예·감정과 일상·속마음 등)은 서로 다른 표현으로 편안하게 풀어 쓰되, 짧은 문장을 뚝뚝 끊어 나열하지 말고 이어지는 내용은 한 문단으로 자연스럽게 잇는다.
...
7. 분량: {length}.
8. 맨 끝 한 문단: 이 풀이가 사주와 별, 두 시계를 나란히 놓고 읽은 것이며, 둘이 왜 같은 자리를 가리키는지·그 이치는 아래 소개하는 책들에 담겨 있다는 담백한 안내(강매 아님). 존재하지 않는 특정 책 제목을 지어내지 말 것.
