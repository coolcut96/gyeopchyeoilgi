...
# 지배 십성 → 사람들 사이에서의 결(관계 축). 관법 용어 없이 쉬운 말로.
RELATION={
 '官':'윗사람·규칙과의 관계에서 신뢰를 얻는 사람. 맡은 책임을 지고 질서를 세우는 자리에서 인정받고, 사람들을 이끄는 역할이 잘 맞는다',
 '財':'사람을 실질적으로 챙기고 주고받는 데 능한 사람. 아랫사람·현장·살림을 잘 건사하고, 실리로 관계를 단단히 만든다',
...
def render(a, tier='유료', time_unknown=False):
    ...
    P.append("---")
    P.append("*이 풀이는 사주와 별, 두 시계를 나란히 놓고 읽었습니다. 둘이 왜 같은 자리를 가리키는지 — 그 이치는 아래 소개하는 책들에 담아두었습니다.*")
    return "\n\n".join(P)
